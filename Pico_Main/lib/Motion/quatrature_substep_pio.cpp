...
void substep_update(substep_state_t *state) {
    ...
    if (state->raw_step != step) {
        ...
        if (!state->stopped)
            state->speed_2_20 = substep_calc_speed(transition_pos - state->prev_trans_pos,
                                                   transition_us - state->prev_trans_us);
    ...
    if (!state->stopped) {
        ...
        if (state->prev_trans_us > state->prev_step_us &&
            ...
            speed_high = substep_calc_speed(state->prev_trans_pos - state->prev_low,
                                            state->prev_trans_us - state->prev_step_us);
            speed_low = substep_calc_speed(state->prev_trans_pos - state->prev_high,
                                           state->prev_trans_us - state->prev_step_us);
        } else {
