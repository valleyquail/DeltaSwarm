...
enum reg : uint8_t {
  ...
  spi_inst_t* PAA5100::get_spi() const {
    return spi;
  }

  int PAA5100::get_cs() const {
    return cs;
  }

  int PAA5100::get_sck() const {
    return sck;
  }

  int PAA5100::get_mosi() const {
    return mosi;
  }

  int PAA5100::get_miso() const {
    return miso;
  }

  int PAA5100::get_int() const {
    return interrupt;
  }

  uint8_t PAA5100::get_id() {
    uint8_t data = 0;
    read_registers(reg::ID, &data, 1);
    return data;
  }

  uint8_t PAA5100::get_revision() {
    uint8_t data = 0;
    read_registers(reg::REVISION, &data, 1);
    return data;
  }

  void PAA5100::set_rotation(Degrees degrees) {
    switch(degrees) {
      default:
      case DEGREES_0:
  ...
  void PAA5100::set_orientation(bool invert_x, bool invert_y, bool swap_xy) {
    uint8_t value = 0;
    if(swap_xy)
      value |= 0b10000000;
    if(invert_y)
  ...
  bool PAA5100::get_motion(int16_t& x_out, int16_t& y_out, uint16_t timeout_ms) {
  ...
  bool PAA5100::get_motion_slow(int16_t& x_out, int16_t& y_out, uint16_t timeout_ms) {
    uint32_t start_time = millis();
    while(millis() - start_time < timeout_ms) {
      uint8_t buf[5];
  ...
  bool PAA5100::frame_capture(uint8_t (&data_out)[FRAME_BYTES], uint16_t& data_size_out, uint16_t timeout_ms) {
    bool success = false;
  ...
  void PAA5100::cs_select() {
    gpio_put(cs, false);  // Active low
  }

  void PAA5100::cs_deselect() {
    sleep_us(1);
    gpio_put(cs, true);
    sleep_us(1);
  }

  void PAA5100::write_register(uint8_t reg, uint8_t data) {
    uint8_t buf[2];
    buf[0] = reg | 0x80;
    buf[1] = data;
    cs_select();
  ...
  void PAA5100::write_buffer(uint8_t *buf, uint16_t len) {
  ...
  void PAA5100::read_registers(uint8_t reg, uint8_t *buf, uint16_t len) {
    cs_select();
  ...
  uint8_t PAA5100::read_register(uint8_t reg) {
    uint8_t data = 0;
    cs_select();
    spi_write_blocking(spi, &reg, 1);
