...
{
    this->pixels.setPixelColor(statusLED_pin, 255, 0, 0);
    this->pixels.show();
}
...
{
    this->pixels.setPixelColor(statusLED_pin, 255, 255, 0);
    this->pixels.show();
}
...
{
    this->pixels.setPixelColor(statusLED_pin, 0, 255, 0);
    this->pixels.show();
#ifdef DEBUG
...
{
    this->pixels.setPixelColor(statusLED_pin, 0, 0, 0);
    this->pixels.show();
#ifdef DEBUG
...
void StatusLED::SetBattery(uint8_t level)
{
    ...
    if (level < 20)
    {
        this->pixels.setPixelColor(batteryLED_pin, 255, 0, 0);
    }
    // set the battery LED to yellow if the battery is below 50%
    else if (level < 50)
    {
        this->pixels.setPixelColor(batteryLED_pin, 255, 255, 0);
    }
    // set the battery LED to green if the battery is above 50%
    else
    {
        this->pixels.setPixelColor(batteryLED_pin, 0, 255, 0);
    }
}
