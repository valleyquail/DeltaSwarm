...
bool pico_i2c_init() {
    ...
    // Preinitialize the flags for the command
    motorCommandBuffer[16] = 'K';
...
void picoSendMovement(float speed, float theta, float omega, bool orientation) {
    ...
    memcpy(&motorCommandBuffer[2], (uint8_t * ) & speed, 4);
    memcpy(&motorCommandBuffer[6], (uint8_t * ) & theta, 4);
    memcpy(&motorCommandBuffer[10], (uint8_t * ) & omega, 4);
#ifdef DEBUG
